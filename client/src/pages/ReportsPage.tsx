...
const ReportsPage: React.FC = () => {
  ...
  // Monthly consumption (mock example)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        {/* Monthly Consumption for Admin & Pharmacist */}
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        {/* Daily Usage */}
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-6">Daily Usage Trends</h3>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={mockUsageData}>
                ...
                {/* Restocked only for Admin & Staff */}
                {(user?.role==='admin'||user?.role==='staff') && (
      ...
      {/* Category & Supplier charts only for Admin */}
      {user?.role==='admin' && (
        <div className="grid lg:grid-cols-2 gap-6">
          {/* Category Value */}
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-6">Value by Category</h3>
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-6">Supplier Orders vs Rating</h3>
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {mockInventoryItems.sort((a,b)=> (b.currentStock*b.price)-(a.currentStock*a.price)).slice(0,5).map(item=>(
                  <tr key={item.id}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{item.name}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">₹{(item.currentStock*item.price).toLocaleString()}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.currentStock} {item.unit}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-4">
            {mockInventoryItems.filter(item=>['low','expired','out-of-stock'].includes(item.status)).slice(0,5).map(item=>(
              <div key={item.id} className="flex items-center justify-between p-3 bg-red-50 rounded-lg">
                <div>
                  <p className="font-medium text-gray-900">{item.name}</p>
                  <p className="text-sm text-gray-600">{item.currentStock} {item.unit} • {item.status.replace('-',' ')}</p>
                </div>
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                  item.status==='expired'?'bg-red-100 text-red-800':
                  item.status==='low'?'bg-yellow-100 text-yellow-800':'bg-gray-100 text-gray-800'}`}>
                  {item.status.replace('-',' ')}
                </span>
              </div>
            ))}
